...
describe("update_task — happy path", () => {
  it("updates a task and returns { ok: true }", async () => {
    ...
    const result = await updateTaskTool.handler({ id: 42, title: "Updated title" }, deps);
  ...
  it("content text mentions task id", async () => {
    ...
    const result = await updateTaskTool.handler({ id: 77, description: "Updated" }, deps);
...
describe("update_task — Zod validation", () => {
  it("rejects when only id is provided (no updatable fields)", async () => {
    const { deps } = buildDeps();

    await expect(updateTaskTool.handler({ id: 42 }, deps)).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects empty input (no id, no fields)", async () => {
    const { deps } = buildDeps();
  ...
  it("rejects extra fields (.strict())", async () => {
    ...
    await expect(
      updateTaskTool.handler({ id: 42, title: "Valid", column_id: 3 }, deps),
    ).rejects.toThrow();
  });
  ...
  it("rejects non-positive id", async () => {
    ...
    await expect(updateTaskTool.handler({ id: 0, title: "X" }, deps)).rejects.toThrow();
    await expect(updateTaskTool.handler({ id: -1, title: "X" }, deps)).rejects.toThrow();
  });

  it("rejects empty title (min 1)", async () => {
    const { deps } = buildDeps();

    await expect(updateTaskTool.handler({ id: 42, title: "" }, deps)).rejects.toThrow();
  });
});
...
describe("update_task — handler error propagation", () => {
  it("propagates KanboardApiError from updateTask", async () => {
    ...
    await expect(
      updateTaskTool.handler({ id: 42, title: "Update" }, deps),
...
describe("update_task — date ISO→epoch conversion", () => {
  it("converts ISO 8601 date_due string to epoch seconds before calling handler", async () => {
    ...
    await updateTaskTool.handler({ id: 42, date_due: isoDate }, deps);
  ...
  it("converts ISO 8601 date_started string to epoch seconds before calling handler", async () => {
    ...
    await updateTaskTool.handler({ id: 42, date_started: isoDate }, deps);
  ...
  it("passes epoch number date_due through unchanged", async () => {
    ...
    await updateTaskTool.handler({ id: 42, date_due: epochSeconds }, deps);
  ...
  it("passes epoch number date_started through unchanged", async () => {
    ...
    await updateTaskTool.handler({ id: 42, date_started: epochSeconds }, deps);
  ...
  it("throws ValidationError for invalid date_due string", async () => {
    ...
    await expect(
      updateTaskTool.handler({ id: 42, date_due: "not-a-date" }, deps),
  ...
  it("throws ValidationError for invalid date_started string", async () => {
    ...
    await expect(
      updateTaskTool.handler({ id: 42, date_started: "garbage-date" }, deps),
    ).rejects.toBeInstanceOf(ValidationError);
  });
});
